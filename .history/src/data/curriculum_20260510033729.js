...
3.  **Rotation propre ($\\phi$)** : rotation du solide sur lui-même.`,
  ...
  }, // --- ÉLECTROMAGNÉTISME L2 : PARTIE 1 (CORRIGÉE) ---
  'l2-em-1': {
    ...
    sections: [
      {
        ...
        content: `La force exercée par une charge $q_1$ sur une charge $q_2$ à une distance $r$ est définie par :
        
$$ \\vec{F}_{1 \\to 2} = \\frac{1}{4\\pi\\epsilon_0} \\frac{q_1 q_2}{r^2} \\vec{u}_{12} $$

Le champ électrique $\\vec{E}$ en un point $M$ est le champ de force par unité de charge. Pour une charge ponctuelle $q$, il s'écrit :
...
$$ \\vec{E}(M) = \\frac{1}{4\\pi\\epsilon_0} \\frac{q}{r^2} \\vec{u}_r $$`,
      },
      {
        ...
        content: `Le flux du champ électrique à travers une surface fermée $S$ est égal à la somme des charges intérieures divisée par la permittivité du vide :

$$ \\Phi = \\oint_S \\vec{E} \\cdot d\\vec{S} = \\frac{Q_{int}}{\\epsilon_0} $$

Ce théorème est fondamental pour calculer le champ $\\vec{E}$ créé par des distributions symétriques (sphères, cylindres ou plans).`,
      }
    ],
    exercises: [
      {
        id: 'ex1',
        title: 'Force de répulsion',
        difficulty: 2,
        statement: "Calculer la force entre deux électrons séparés de $10^{-10}$ m.",
        solution: "En utilisant $q = -1,6 \\cdot 10^{-19}$ C, on applique la formule de Coulomb : $F \\approx 2,3 \\cdot 10^{-8}$ N."
      }
    ]
  },
}
