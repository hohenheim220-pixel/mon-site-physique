...
3.  **Rotation propre ($\\phi$)** : rotation du solide sur lui-même.`,
  ...
  }, // --- ÉLECTROMAGNÉTISME L2 : PARTIE 1 (ID: l2-em-1) ---
  'l2-em-1': {
    ...
    intro: "Cette leçon pose les deux piliers de l'électrostatique : l'interaction locale entre charges (Coulomb) et la vision globale du flux électrique (Gauss).",
    sections: [
      {
        id: 's1',
        title: '1. Loi de Coulomb et Force Électrostatique',
        content: `La force exercée par une charge $q_1$ sur une charge $q_2$ séparées par une distance $r$ est :
$$\\vec{F_{1 \\to 2}} = \\frac{1}{4\\pi\\epsilon_0} \\frac{q_1 q_2}{r^2} \\vec{u}_{12}$$
- Si les charges sont de même signe, la force est **répulsive**.
- Si elles sont de signes opposés, elle est **attractive**.
L'unité de la constante $\\epsilon_0$ (permittivité du vide) est le $F\\cdot m^{-1}$.`,
      },
      {
        id: 's2',
        title: '2. Le Champ Électrique $\\vec{E}$',
        content: `Une charge $q$ crée en tout point de l'espace un champ électrique. Si on place une charge test $q_0$ en ce point, elle subit la force $\\vec{F} = q_0 \\vec{E}$.
        
Pour une charge ponctuelle :
$$\\vec{E} = \\frac{1}{4\\pi\\epsilon_0} \\frac{q}{r^2} \\vec{u}_r$$
Le champ est toujours dirigé vers l'extérieur pour une charge positive.`,
      },
      {
        id: 's3',
        title: '3. Théorème de Gauss',
        content: `Le flux du champ électrique à travers une surface fermée $S$ est proportionnel à la somme des charges enfermées :
$$\\Phi = \\oint_S \\vec{E} \\cdot d\\vec{S} = \\frac{\\sum Q_{int}}{\\epsilon_0}$$
**Utilité :** Ce théorème permet de calculer le champ $\\vec{E}$ de distributions complexes (fils, plans, sphères) sans passer par des intégrales de Coulomb difficiles.`,
      }
    ],
    exercises: [
      {
        id: 'ex1',
        title: 'Exercice : Équilibre de deux charges',
        difficulty: 3,
        statement: "Deux charges $q$ identiques sont fixées à une distance $d$. Où faut-il placer une troisième charge $Q$ pour qu'elle soit en équilibre (force nulle) ?",
        solution: "Par symétrie, la force totale sera nulle exactement au milieu des deux charges ($d/2$). En ce point, les vecteurs champs électriques créés par chaque charge $q$ sont égaux en module mais opposés en direction, s'annulant mutuellement."
      },
      {
        id: 'ex2',
        title: 'Exercice : Flux à travers un cube',
        difficulty: 4,
        statement: "Une charge $q$ est placée au centre d'un cube. Quel est le flux du champ électrique à travers une seule des faces du cube ?",
        solution: "Le flux total à travers les 6 faces est $\\Phi_{tot} = q/\\epsilon_0$ (Gauss). Par symétrie, le flux à travers une seule face est le sixième du flux total, soit : $\\Phi_{face} = \\frac{q}{6\\epsilon_0}$."
      }
    ]
  },
}
